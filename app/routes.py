...
@main.route("/billing-reports")
def billing_reports():
    return render_template("reports/billing_reports.html")
...
@main.route("/billing-reports/data")
def billing_report_data():
    ...
    try:
        ...
        bill_query = (
            ...
            .outerjoin(
                Department,
                Department.department_name == Patient.department
            )
        )
        ...
        if payment_type:
            bill_query = bill_query.filter(
                Bill.pay_type.ilike(payment_type)
            )
        ...
        for bill, patient, department in bill_results:
        ...
        refund_query = db.session.query(BillRefund)
        ...
        for refund in refunds:
            ...
            refund_data.append({
                ...
                "time": (
                    refund.created_at.strftime("%I:%M %p")
                    if refund.created_at
                    else ""
                ),

                "hospital_no": refund.patient_no or "",

                "patient_name": refund.patient_name or "",
        ...
        deposit_query = db.session.query(Deposit)
        ...
        for deposit in deposits:
            ...
            deposit_data.append({
                ...
                "time": (
                    deposit.deposit_date.strftime("%I:%M %p")
                    if deposit.deposit_date
                    else ""
                ),

                "hospital_no": deposit.patient_no or "",

                "patient_name": deposit.patient_name or "",

                "amount": amount,

                "remarks": deposit.remarks or ""
            })
