document.addEventListener('DOMContentLoaded', function () {
  ...
  // ==========================================================
  // TEST CATALOG - LOAD FROM DATABASE
  // ==========================================================
  ...
  // ----------------------------------------------------------
  // Load active tests from Flask database
  // ----------------------------------------------------------
  ...
  function bindEntryRowEvents(tr) {
    ...
    const codeInput =
      tr.querySelector('[data-field="code"]');

    const nameInput =
      tr.querySelector('[data-field="name"]');

    const priceInput =
      tr.querySelector('[data-field="price"]');

    const qtyInput =
      tr.querySelector('[data-field="qty"]');

    const discInput =
      tr.querySelector('[data-field="discPct"]');

    const addBtn =
      tr.querySelector('.btn-add-row');
    ...
    addBtn.addEventListener('click', function () {
      ...
      if (
        ...
        alert(
          'Search and select a test before adding.'
        );
      ...
      // Make sure selected test actually exists
...
if (!selectedTestId) {
  ...
  function confirmRow(tr) {
    ...
    tr.querySelectorAll('input').forEach((input) => {
      ...
      if (
        input.dataset.field === 'qty' ||
        input.dataset.field === 'discPct'
      ) {
  ...
  function recalcRow(tr) {
    ...
    const discPct =
      parseFloat(
        tr.querySelector('[data-field="discPct"]').value
      ) || 0;
  ...
  function fetchPatientData() {
    ...
    fetch(
      `/fetch_patient/${encodeURIComponent(patientNo)}`
    )
    ...
    .addEventListener(
      ...
      function () {
        ...
        if (items.length === 0) {
          ...
          alert(
            'Add at least one test before saving.'
          );
        ...
        const payload = {
          ...
          age_sex:
            document
              .getElementById('patient_age_sex')
              .value.trim(),
          ...
          rate_type:
            document
              .getElementById('rate_type')
              .value,

          referring_doctor:
            document
              .getElementById('referring_doctor')
              .value.trim(),
  ...
  async function initializeBilling() {
    ...
    // First load tests from database
    await loadTests();

    // Then create the first empty row
    createEntryRow();
